...
export default Ember.Controller.extend(GeoLocationMixin, {
  ...
  errorMessage: null,
    ...
    this.get('geolocation').getGeoposition().then(function(geoposition) {
      ...
      Ember.$.get('https://maps.googleapis.com/maps/api/geocode/json?latlng=41.4031163,2.181871', { latlng: geoposition.coords.latitude + "," + geoposition.coords.longitude }).then(function(data) {
	...
	findMunicipality: function(geoposition, values, currentIndex, maxIndex, _this) {
		...
		if (value.types.indexOf("political") > -1) {
  		_this.store.find('municipality', { name: value.long_name, longitude: geoposition.coords.longitude, latitude: geoposition.coords.latitude }).then(function(response) {
  			// municipality found
  			var municipality = response.get('firstObject')._data;
  			// find all municipalities for this province
  			_this.store.find('municipality', { provinceId: municipality.provinceId }).then(function(response) {
  			...
  			_this.store.find('province', municipality.provinceId).then(function(response) {
  				...
  				_this.store.find('province', { regionId: province.regionId }).then(function(response) {
  ...
  findProvinces: function() {
  	if (this.get('isInitialised')) {
	  	...
	  	if (regionId > 0) {
	  		var _this = this;
	  		this.store.find('province', { regionId: regionId }).then(function(response) {
				_this.set('provinces', response);
				_this.set('provincesEnabled', true);
			});
	  	}
  ...
  findMunicipalities: function() {
  	if (this.get('isInitialised')) {
	  	...
	  	if (provinceId > 0) {
	  		var _this = this;
	  		this.store.find('municipality', { provinceId: provinceId }).then(function(response) {
				  _this.set('municipalities', response);
				  _this.set('municipalitiesEnabled', true);
			  });
	    }
